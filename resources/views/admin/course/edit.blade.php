                            ...
                            <form method="POST" action="{{ route('course.update',[$course->id]) }}">
                                ...
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <input type="number" class="form-control" id="exampleInputEmail1" name="fees"
                                            placeholder="Enter Fees" value="{{ $course->fees }}">
                                    </div>
                                </div>
                                ...
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <?php $shiftIds = json_decode($course->shifts); ?>
                                        <select name="shifts[]" class="select2 form-control" multiple>
                                            @foreach($shifts as $shift)
                                            <option @if(in_array($shift->id,$shiftIds)) selected="selected" @endif value="{{ $shift->id }}">{{ $shift->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
